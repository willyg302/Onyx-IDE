 ...
 * <li>vfill - Extent component vertically (only one line allowed per container) //$rh
 ...
 * @author David Ekholm
 *
 * 2005-05-23 David Ekholm
 * Bugfix: JScrollPanes were oversized (sized to their containing component)
 * if the container containing the JScrollPane was resized.
 *
 * 2006-11-13 Robert Hetzel (www.rhetzel.ch)
 * Various enhancements concerning vertical alignment, especially with "vfill", but also other
 * improvements.
 */
public class RiverLayout extends FlowLayout implements LayoutManager, java.io.Serializable {
    ...
    public RiverLayout(int hgap, int vgap) {
        this.hgap = hgap;
        this.vgap = vgap;
        setExtraInsets(new Insets(vgap, hgap, vgap, hgap));
    }
    ...
    boolean isFirstInRow(Component comp) {
        ...
        return (cons != null) &&
        ((cons.indexOf(RiverLayout.LINE_BREAK) != -1) ||
    ...
    public Dimension preferredLayoutSize(Container target) {
        synchronized (target.getTreeLock()) {
            ...
            for (int i = 0; i < nmembers; i++) {
                ...
                if (hasConstraint(comp, TAB_STOP)) {
    ...
    protected void moveComponents(Container target, int x, int y, int width, int height,
        ...
        synchronized (target.getTreeLock()) {
            ...
            for (int i = rowStart; i < rowEnd; i++) {
                ...
                if (valign == VTOP) {
                    dy = 0;
                } else if (valign == VBOTTOM) {
                    dy = height - m.getHeight();
                } else {
                    dy = (height - m.getHeight()) / 2;
                }
    ...
    protected void adjustAlignment(Component m) {
        ...
        if (hasConstraint(m, RiverLayout.VTOP)) {
            ...
            valign = VCENTER;
        } else if (hasConstraint(m, RiverLayout.VBOTTOM)) {
            valign = VBOTTOM;
        }
    }
    ...
    public void layoutContainer(Container target) {
        ...
        synchronized (target.getTreeLock()) {
            ...
            for (int i = vfillStart; i < moveDownStart; i++) {
                Component m = target.getComponent(i);
                if (!m.isVisible()) continue;
                rowh = Math.max(rowh, m.getHeight());
            }
            ...
            if (yslackMax > Integer.MIN_VALUE) {
                relMove(target, 0, yslackMax, moveDownStart, nmembers);
            }
        }
    }
}
